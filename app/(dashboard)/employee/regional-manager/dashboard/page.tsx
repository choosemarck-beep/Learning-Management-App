...
export default async function RegionalManagerDashboardPage() {
  console.log("[RegionalManagerDashboard] Page component started");
  ...
  try {
    ...
    console.log("[RegionalManagerDashboard] User retrieved:", { id: user?.id, role: user?.role });
  } catch (authError) {
    console.error("[RegionalManagerDashboard] Error getting current user:", authError);
  ...
  if (!user) {
    console.error("[RegionalManagerDashboard] User is null - middleware should have prevented this");
  ...
  if (user.role !== "REGIONAL_MANAGER") {
    console.log("[RegionalManagerDashboard] User role mismatch:", user.role);
    ...
    console.log("[RegionalManagerDashboard] Redirecting to:", redirectUrl);
  ...
  try {
    ...
    try {
      console.log(`[RegionalManagerDashboard] Fetching user data for user ID: ${user.id}`);
      ...
      console.log(`[RegionalManagerDashboard] User data fetched successfully, courses: ${userData?.courseProgresses?.length || 0}`);
    } catch (dbError) {
      ...
      console.error("[RegionalManagerDashboard] Error fetching user data:", {
        userId: user.id,
        userRole: user.role,
        error: errorDetails,
    ...
    if (!userData) {
      console.error("[RegionalManagerDashboard] User data is null after fetch");
  ...
  try {
    ...
    if (!user.name || !user.email) {
      console.error("[RegionalManagerDashboard] User missing required fields:", {
        hasName: !!user.name,
        hasEmail: !!user.email,
        userId: user.id,
        userRole: user.role,
    ...
    const safeUserName = String(user.name || 'Regional Manager');
    
    console.log("[RegionalManagerDashboard] Final render check:", {
      hasUser: !!user,
    ...
    console.error("[RegionalManagerDashboard] CRITICAL ERROR:", {
