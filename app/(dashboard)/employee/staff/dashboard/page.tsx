...
export default async function StaffDashboardPage() {
  console.log("[StaffDashboard] Page component started");
  ...
  try {
    ...
    console.log("[StaffDashboard] User retrieved:", { id: user?.id, role: user?.role });
  } catch (authError) {
    console.error("[StaffDashboard] Error getting current user:", authError);
  ...
  if (!user) {
    console.error("[StaffDashboard] User is null - middleware should have prevented this");
  ...
  if (user.role !== "EMPLOYEE") {
    console.log("[StaffDashboard] User role mismatch:", user.role);
    ...
    console.log("[StaffDashboard] Redirecting to:", redirectUrl);
  ...
  try {
    ...
    try {
      console.log(`[StaffDashboard] Fetching user data for user ID: ${user.id}`);
      ...
      console.log(`[StaffDashboard] User data fetched successfully, courses: ${userData?.courseProgresses?.length || 0}`);
    } catch (dbError) {
      ...
      console.error("[StaffDashboard] Error fetching user data:", {
        userId: user.id,
        userRole: user.role,
        error: errorDetails,
    ...
    if (!userData) {
      console.error("[StaffDashboard] User data is null after fetch");
  ...
  try {
    ...
    if (!user.name || !user.email) {
      console.error("[StaffDashboard] User missing required fields:", {
        hasName: !!user.name,
        hasEmail: !!user.email,
        userId: user.id,
        userRole: user.role,
    ...
    const safeUserName = String(user.name || 'Employee');
    
    console.log("[StaffDashboard] Final render check:", {
      hasUser: !!user,
    ...
    console.error("[StaffDashboard] CRITICAL ERROR:", {
