...
export default async function BranchManagerDashboardPage() {
  console.log("[BranchManagerDashboard] Page component started");
  ...
  try {
    ...
    console.log("[BranchManagerDashboard] User retrieved:", { id: user?.id, role: user?.role });
  } catch (authError) {
    console.error("[BranchManagerDashboard] Error getting current user:", authError);
  ...
  if (!user) {
    console.error("[BranchManagerDashboard] User is null - middleware should have prevented this");
  ...
  if (user.role !== "BRANCH_MANAGER") {
    console.log("[BranchManagerDashboard] User role mismatch:", user.role);
    ...
    console.log("[BranchManagerDashboard] Redirecting to:", redirectUrl);
  ...
  try {
    ...
    try {
      console.log(`[BranchManagerDashboard] Fetching user data for user ID: ${user.id}`);
      ...
      console.log(`[BranchManagerDashboard] User data fetched successfully, courses: ${userData?.courseProgresses?.length || 0}`);
    } catch (dbError) {
      ...
      console.error("[BranchManagerDashboard] Error fetching user data:", {
        userId: user.id,
        userRole: user.role,
        error: errorDetails,
    ...
    if (!userData) {
      console.error("[BranchManagerDashboard] User data is null after fetch");
  ...
  try {
    ...
    if (!user.name || !user.email) {
      console.error("[BranchManagerDashboard] User missing required fields:", {
        hasName: !!user.name,
        hasEmail: !!user.email,
        userId: user.id,
        userRole: user.role,
    ...
    const safeUserName = String(user.name || 'Branch Manager');
    
    console.log("[BranchManagerDashboard] Final render check:", {
      hasUser: !!user,
    ...
    console.error("[BranchManagerDashboard] CRITICAL ERROR:", {
