...
- User → CourseProgress[] (many-to-many with Course)
- User → TaskCompletion[] (many-to-many with Task)
- User → Badge[] (one-to-many)
- User → Session[] (one-to-many, NextAuth)
- Course → Module[] (one-to-many)
- Course → CourseProgress[] (many-to-many with User)
- Module → Course (many-to-one)
- Module → Lesson[] (one-to-many)
- Lesson → Module (many-to-one)
- Lesson → Task[] (one-to-many)
- Task → Lesson (many-to-one)
- Task → TaskCompletion[] (many-to-many with User)
...
**Selected Service: Resend** (evaluated December 2024)

**Why Resend?**
- Free tier: 3,000 emails/month (sufficient for onboarding emails)
...
1. **Postmark**: $10/month for 10,000 emails - Best deliverability, transactional focus
2. **SendGrid**: Free tier (100/day), more complex, established service
3. **Mailgun**: Free for 3 months, then $35/month - Developer-friendly
4. **Amazon SES**: $0.10 per 1,000 emails - Cost-effective at scale, complex setup
5. **Nodemailer + SMTP**: Free but requires SMTP provider, not ideal for production
...
- ❌ **NEVER** use: "You haven't completed this yet"
